...
// (Same queries as in generate_summary_pdf.php)
...
$spreadsheet->getProperties()
    ...
    ->setTitle("Income Summary Report")
    ->setDescription("Summary report of income and bookings");
...
// Income Summary
$sheet->setCellValue('A4', 'Income Summary');
$sheet->mergeCells('A4:B4');
$sheet->getStyle('A4')->getFont()->setBold(true);

$sheet->setCellValue('A5', 'Total Income:');
$sheet->setCellValue('B5', '₱' . number_format($total_income, 2));
$sheet->getStyle('B5')->getFont()->setBold(true);

// By Branch
$sheet->setCellValue('A7', 'By Branch:');
$sheet->mergeCells('A7:B7');
$sheet->getStyle('A7')->getFont()->setBold(true);

$row = 8;
foreach ($branch_incomes as $branch_name => $income) {
    $sheet->setCellValue('A' . $row, $branch_name);
    $sheet->setCellValue('B' . $row, '₱' . number_format($income, 2));
    $row++;
}

// By Service
$sheet->setCellValue('A' . ($row + 1), 'By Service:');
...
$sheet->getStyle('B5:B5')->applyFromArray($amountStyle);
